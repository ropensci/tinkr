...
test_that("markdown can be appended to elements", {
  ...
  txt <- c(
    "The following message is sponsored by me:\n",
    "> Hello from *tinkr*!",
    ">",
    ">  :heart: R"
  )
...
test_that("Inline markdown can be appended (to a degree)", {
  ...
  nodes <- xml2::xml_find_all(
    ex$body,
    ".//md:code[contains(text(), 'READ THIS')]",
    ex$ns
  )
  ...
  nodes <- xml2::xml_find_all(
    ex$body,
    ".//md:code[text()='<-- READ THIS']/preceding-sibling::md:text[text()=' ']",
    ex$ns
  )
  ...
  expect_equal(
    tst$append_md("**oh hai**", ".//md:emph")$show()[1],
...
test_that("space parameter can be shut off", {
  path <- system.file("extdata", "example2.Rmd", package = "tinkr")
  ex <- tinkr::yarn$new(path)
  chk <- xml2::xml_find_all(
    ex$body,
    ".//md:heading/*[contains(text(), '!!!')]",
    ex$ns
  )
  space_chk <- xml2::xml_find_all(
    ex$body,
    ".//md:heading/*[contains(text(), ' !!!')]",
    ex$ns
  )
...
test_that("markdown can be prepended", {
  ...
  nodes <- xml2::xml_find_all(
    ex$body,
    ".//node()[contains(text(), 'NERDS')]",
    ex$ns
  )
  expect_length(nodes, 0)
  ex$prepend_md(
    "I come before the table.\n\nTable: BIRDS, NERDS",
    ".//md:table"
  )
  nodes <- xml2::xml_find_all(
    ex$body,
...
test_that("inline markdown can be prepended", {
  ...
  #> *hello*? how are you?
  expect_equal(
    tst$prepend_md("**oh hai**", ".//md:emph")$show()[1],
...
test_that("md_vec() will convert a query to a markdown vector", {
  ...
  expected <- paste(
    strrep("#", xml2::xml_attr(headings, "level")),
